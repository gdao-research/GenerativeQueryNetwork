...
_DATASETS = dict(data=DatasetInfo(basepath='data', train_size=1, test_size=1, frame_size=64, sequence_size=4000))
_NUM_CHANNELS = 3
...
def _read_numpy_data(root, dataset, mode='train'):
    ...
    for f in img_files:
        ...
        pose = poses[coded]
        _p = [pose[0]/10., pose[1]/10., pose[2]/10.]
        _p += [np.sin(pose[3]), np.cos(pose[3])]
        _p += [np.sin(pose[4]), np.cos(pose[4])]
        _p += [np.sin(pose[5]), np.cos(pose[5])]
        _imgs.append(img)
        _poses.append(_p)
...
class DataReader(tf.data.Dataset):
    def __init__(self, dataset, context_size, root, mode='train', custom_frame_size=None, num_threads=4, buffer_size=256, parse_batch_size=32):
        ...
        self._dataset = tf.data.Dataset.from_generator(_generator, (tf.float32, tf.float32), (tf.TensorShape([self._example_size, 64, 64, 3]), tf.TensorShape([self._example_size, 9])), args=(imgs, poses, self._example_size))
...
def input_fn(dataset, context_size, root, mode, batch_size=1, num_epochs=1, custom_frame_size=None, num_threads=4, buffer_size=256, seed=None):
    ...
    dataset = DataReader(dataset, context_size, root, str_mode, custom_frame_size, num_threads, buffer_size)
    if mode == tf.estimator.ModeKeys.TRAIN:
...
if __name__ == '__main__':
    [Qu, Ta] = input_fn('data', 5, '/home/gdao/Dropbox (UNC Charlotte)/Fall18/computer_vision/final', tf.estimator.ModeKeys.PREDICT, batch_size=5)
