...
def main():
    ...
    if C.INITAIL_EVAL:
        eval_input = lambda: input_fn(dataset=C.DATASET, context_size=C.CONTEXT_SIZE, root=C.DATA_DIR, mode=tf.estimator.ModeKeys.EVAL, batch_size=C.BATCH_SIZE, num_threads=C.QUEUE_THREAD, buffer_size=C.QUEUE_BUFFER)
        eval_results = classifier.evaluate(input_fn=eval_input, hooks=[logging_hook])

    for _ in range(C.TRAIN_EPOCHS):
        train_input = lambda: input_fn(dataset=C.DATASET, context_size=C.CONTEXT_SIZE, root=C.DATA_DIR, mode=tf.estimator.ModeKeys.TRAIN, batch_size=C.BATCH_SIZE, num_threads=C.QUEUE_THREAD, buffer_size=C.QUEUE_BUFFER)
